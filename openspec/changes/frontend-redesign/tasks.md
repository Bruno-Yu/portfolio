...
- [ ] 2.1 從目前 main working tree 建立 `feat/design` 分支：`git checkout -b feat/design`
- [ ] 2.2 將 FrontStage design 改動、Spectra 文件、設定檔 commit 至 `feat/design`（不含 `workers/`）
- [ ] 2.3 Push `feat/design` 至 remote：`git push -u origin feat/design`

## 3. 待確認 — 功能驗證（feat/design 分支）

<!-- 在 feat/design 建立後本地 pnpm dev 逐項確認 -->

- [ ] 3.1 本地 `pnpm dev` 驗證：首頁 7 個 section 正常顯示，無 console error
- [ ] 3.2 驗證 Rail sidebar 在 desktop 固定、mobile 收為 top bar
- [ ] 3.3 驗證語言 / 主題切換，刷新後保持設定（無 FOUC）
- [ ] 3.4 驗證 scroll-spy：捲動至各 section，Rail nav active 正確更新
- [ ] 3.5 驗證 Works section 首頁 modal：點擊卡片有圖片、描述、連結
- [ ] 3.6 驗證 Works 獨立頁面：分頁正常，modal 功能正常
- [ ] 3.7 驗證 Login 頁面：雙欄版型，表單可正常登入
...
- [ ] 4.1 依照 Portfolio copy is treated as resume-adjacent content 原則重寫 Hero 中文主文案：定位為「前端工程師，具全端協作能力」，保留作品集設計感，不寫成履歷條列。移除或替換「誠實的前端介面」、「可接案 · Q3 2026」等過度風格化或接案導向訊號。
- [ ] 4.2 調整 Hero CTA、quick tags、底部三張 quick-nav 卡片文案：保留原本數量與版面結構，讓使用者能快速前往作品、關於、聯絡資訊。
- [ ] 4.3 重寫 About 中文三段式內容：第一段說明目前職能、年資、主要技術與工作場景；第二段將越南 B2B 業務背景轉化為溝通、跨文化協作、需求釐清與交付意識；第三段描述工程習慣，例如讀文件、拆分元件、維護既有系統、補文件與協助團隊。移除「不掉鏈子」、「瑣事」等不適合履歷篩選情境的措辭。
- [ ] 4.4 檢查 About facts：確認現職、主要技術、TOEIC、求職方向、語言能力與正式履歷一致；若正式履歷不同，以正式履歷為準。
- [ ] 4.5 重寫 Services 中文文案：維持三張卡片與 bullet 結構，將「我能解決什麼」改為更職場導向但不僵硬的表述，例如核心能力或可支援工作；內容聚焦前端工程、UI 實作、全端協作。
- [ ] 4.6 重寫 Works seed data 的 `description` 與 `content`：更新 `data-migration/db.json`，並同步 canonical seed SQL。避免「仿做」、「順便鍛鍊」、「練習作品」等弱化成果的說法；每個作品用短句交代目的、技術、實作重點與可檢視成果。
- [ ] 4.7 修正作品資料中的技術名稱與 typo：統一 `TypeScript`、`Vue CLI`、`RESTful API` 等拼法；修正 `RESTful AP`。
- [ ] 4.8 調整 Contact 中文文案：保留作品集的開放聯絡感，不寫成接案廣告，也不過度求職化；建議語氣為「若想了解作品細節、技術合作或職缺媒合，歡迎聯絡」。
- [ ] 4.9 更新 `frontend/index.html` SEO / OG / title：改為正式作品集摘要，避免仍使用舊專案「以 React 為核心框架...熟練技術」的描述。
- [ ] 4.10 更新 `frontend/README.md`：同步年資、職稱、技術定位與聯絡語氣，避免 README 寫「近兩年」但首頁寫「三年」。
...
<!--
分支策略：
  - 若 Admin 設計稿在 feat/design merge 前完成 → 直接在 feat/design 上作業
  - 若較晚完成 → 從 feat/design 開新分支 feat/admin-redesign，merge 目標為 feat/design
-->

- [ ] 6.1 取得 Admin BackStage 設計稿（目前尚無）
- [ ] 6.2 **[需人工確認]** 決定分支策略：在 `feat/design` 上直接作業 或 另開 `feat/admin-redesign`
- [ ] 6.3 實作 Admin 新版 layout 與導覽
- [ ] 6.4 實作 works 列表頁新 UI
- [ ] 6.5 實作 works create/edit 表單新 UI — BackStage 圖片欄位必須採用 R2 邏輯（merge 前提）：image 欄位必須使用 `POST /api/upload`（上傳）和 `DELETE /api/upload/:key`（刪除），禁止恢復 Imgur 手動 URL 輸入
- [ ] 6.6 驗證 BackStage 圖片上傳 / 刪除在新 Admin UI 下正常運作
- [ ] 6.7 若另開了 `feat/admin-redesign` 分支：merge 回 `feat/design` 後刪除此分支

## 7. Pre-Merge Checklist（feat/design → main）

<!--
Merge 前提：
  1. main 上 image-storage-r2 功能已確認可用且 spectra archive 已執行
  2. Sections 1–6 所有 tasks 完成（含人工確認）
  3. feat/admin-redesign（若有）已 merge 回 feat/design
-->

- [ ] 7.1 確認 Sections 1–6 所有 tasks（含人工確認項目）完成
- [ ] 7.2 確認 `main` 上 `spectra archive image-storage-r2` 已執行（R2 upload/delete 在生產環境確認可用）
...
- [ ] 7.3a 執行 merge 策略（merge 策略）：`git checkout main && git merge feat/design --no-ff`
- [ ] 7.4 解決 `workers/wrangler.toml` 衝突：保留兩邊（`compatibility_date` from feat/design + `[[r2_buckets]]` binding from main）
- [ ] 7.5 解決 `workers/src/index.ts` 衝突：保留兩邊（CORS origin fix from feat/design + `IMAGES_BUCKET: R2Bucket` + upload router from main）
- [ ] 7.6 確認 merge 後 `frontend/src/views/BackStage/` 使用 R2 邏輯（main 版本），未被 design 分支覆蓋
